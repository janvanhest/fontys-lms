...
echo "========================================"
echo " bench-chat.sh — totale tijd"
...
for round in $(seq 1 $ROUNDS); do
  ...
  round_avg=$(echo "scale=2; $round_total / $CONCURRENT" | bc)
...
echo "========================================"
echo " Eindresultaat (${#results[@]} requests, sleep uitgesloten)"
echo "========================================"
total=0
min=${results[0]}
max=0
...
avg=$(echo "scale=2; $total / ${#results[@]}" | bc)
printf "  min: %ss\n  avg: %ss\n  max: %ss\n" "$min" "$avg" "$max"
echo "========================================"
